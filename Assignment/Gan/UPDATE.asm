...
SEARCH_BOOK:
        CMP BL, BOOK_ID_ARRAY[SI]      ;GET THE BOOK ID INDEX NUMBER
        JE BOOK_FOUND

        INC SI
    LOOP SEARCH_BOOK

    MOV AH, 09H                     ;IF BOOK NOT FOUND
    LEA DX, BOOKNOTFOUND
    INT 21H
...
BOOK_FOUND:
    ...
    ; ASK THEM TO EDIT NAME OR AUTHOR
    MOV AH , 09H
...
EDIT_NAME: 
    ...
    ;CALCALUTE THE INDEX OF THE BOOK NAME
    MOV AX, SI              ; SI IS THE INDEX OF THE BOOK ID
    MOV CX , 50             ; 50 IS THE LENGTH OF THE BOOK NAME
    MUL CX                  ; NEED TO MULTIPLY BY 50 CUZ 50 BYTE FOR EACH BOOK NAME

    LEA DI , BOOK_NAME_ARRAY    ; POINT TO THE STARTING POINT OF THE BOOK NAME ARRAY
    ADD DI , AX                 ; ADD THE INDEX OF THE BOOK NAME

    MOV CX, 50                  ; SET THE LENGTH OF THE BOOK NAME
    ...
    LEA SI , EDITEDBOOKNAME  ;POINT TO THE STARTING POINT OF THE EDITED BOOK NAME
    ...
    ;POINT BACK TO THE START OF THE EDITED BOOK NAME
    XOR AH, AH      
    SUB SI, AX
...
BOOK_ID_VALIDATION PROC
    ...
    CMP INPUTBUFFER[1], 1       ; CHECK THE TOTAL INPUT LENGTH
    JE CHECK_ONE_CHARACTER      ; IF ITS ONE

    CMP INPUTBUFFER[1], 2       ; CHECK THE TOTAL INPUT LENGTH
    JE CHECK_TWO_CHARACTER      ; IF ITS TWO

    JMP INVALID_INPUT_BOOK_ID

CHECK_ONE_CHARACTER: 
    CMP INPUTBUFFER[2], "1"     ; OUTPUT[0] /  CHECK IF ITS LESS THAN 1 
    JL INVALID_INPUT_BOOK_ID

    CMP INPUTBUFFER[2], "9"     ; OUTPUT[0] / CHECK IF ITS GREATER THAN 9
...
CHECK_TWO_CHARACTER: 
    ...
    CMP INPUTBUFFER[2], "1"         ;OUTPUT[0] / CHECK IF ITS 1
    JE CHECK_10_TO_19               ; CAN ONLY ACCEPT 10 TO 19

    CMP INPUTBUFFER[2], "2"         ;OUTPUT[0] / CHECK IF ITS 2
    JE CHECK_20                     ; CAN ONLY ACCEPT 20

    JMP INVALID_INPUT_BOOK_ID


CHECK_10_TO_19:
    CMP INPUTBUFFER[3], "0"         ;OUTPUT[1] / CHECK IF ITS  LESS 0
    JL INVALID_INPUT_BOOK_ID

    CMP INPUTBUFFER[3], "9"         ;OUTPUT[1] /  CHECK IF ITS GREATER THAN 9
    JG INVALID_INPUT_BOOK_ID

    RET

CHECK_20:
    CMP INPUTBUFFER[3], "0"         ;OUTPUT[1] / CHECK IF ITS 0 -  CAN ONLY ACCEPT 20  
    JE VALID_INPUT
...
INPUT_VALIDATION_LENGHT_EMPTY PROC
    CMP NEWBOOKNAME[2] , 0DH            ; CHECK IF THE INPUT IS EMPTY , ODH IS ENTER KEY
    JE EMPTY_INPUT_HANDLE               ; IF EMPTY

    MOV AL , MAXN_BOOKNAME              ; CHECK THE MAXIMUM LENGTH OF THE INPUT
    CMP ACTN_BOOKNAME , AL              ; IF THE INPUT IS GREATER THAN THE MAXIMUM LENGTH
    JG LENGTH_INPUT_HANDLE              ; IF GREATER

    CMP NEWAUTHORNAME[2] , 0DH          ; CHECK IF THE INPUT IS EMPTY , ODH IS ENTER KEY
    JE EMPTY_INPUT_HANDLE               ; IF EMPTY

    MOV AL , MAXN_AUTHOR                ; CHECK THE MAXIMUM LENGTH OF THE INPUT
    CMP ACTN_AUTHOR , AL                ; IF THE INPUT IS GREATER THAN THE MAXIMUM LENGTH
    JG LENGTH_INPUT_HANDLE              ; IF GREATER
